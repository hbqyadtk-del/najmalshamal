...
// === المتغيرات الجديدة للحضور ===
let attendanceData = loadFromStorage('attendanceData') || {};
let currentWeekKey = ''; // ← تم نقله إلى الأعلى
...
function login() {
  ...
  if (u && p === "590") navigateTo("loginPage", "menuPage");
  else alert("البيانات غير صحيحة!");
}

// === الحقول المخصصة للطلاب ===
...
function renderTeachersByClass() {
  ...
  classes.forEach(cls => {
    ...
    const allKeys = new Set();
    list.forEach(t => Object.keys(t).forEach(k => {
      if (k !== 'الصف' && k !== 'نوع_الراتb') {
        allKeys.add(k);
      }
    }));
    const keys = Array.from(allKeys);
    ...
    list.forEach((teacher, index) => {
      ...
      actionsCell.innerHTML = `
        <button class="btn btn-sm" onclick="editTeacher('${cls}', ${index})">✏️</button>
        <button class="btn btn-sm btn-danger" onclick="deleteTeacherPermanent('${cls}', ${index})">🗑️</button>
        <button class="btn btn-sm btn-danger" onclick="deleteTeacher('${cls}', ${index})">❌</button>
      `;
    });
  ...
  if (searchInput) {
    ...
    searchInput.oninput = () => {
      ...
      const tables = document.querySelectorAll('#teachersTablesContainer table');
...
// دالة حذف نهائي (للمعلمين فقط - اختياري)
...
// === دالة editTeacher المُصلحة نهائياً (لا تكرار أبدًا) ===
function editTeacher(cls, index) {
  const t = teachers[cls][index];
  currentEditTeacher = { cls, index };
  navigateTo("teachersByClassPage", "addTeacherPage");
  
  // ✅ مسح كامل لجميع الحقول المخصصة قبل إعادة إنشائها
  const container = document.getElementById('customTeacherFields');
  container.innerHTML = '';
  ...
  // ✅ عرض الحقول المخصصة المعرفة فقط (بدون أي تكرار)
...
// دالة عرض/تعديل أسبوع من صفحة الشهر
...
// === فتح أسبوع جديد (من زر "الأسبوع") ===
function showWeeklyAttendance() {
  const now = new Date();
  const day = now.getDay(); // الأحد = 0
  ...
  renderWeeklyAttendance(weekKey, weekStart, weekEnd, true); // true = قابل للتعديل
}
...
function renderWeeklyAttendance(weekKey, startDate, endDate, isEditable = false) {
  ...
  currentWeekKey = weekKey; // ← ✅ هذا السطر في مكانه الصحيح الآن

  // أخفاء زر الجرد إذا لم يكن في وضع التحرير
...
function toggleDayState(weekKey, teacherName, dayIndex) {
  ...
  saveToStorage('attendanceData', attendanceData);
  // إعادة عرض (نحتاج تواريخ من العنوان)
...
// === عرض الأسابيع المُجرَّدة فقط (من زر "الشهر") ===
function showMonthlyAttendance() {
  ...
  // عرض فقط الأسابيع التي تم جردها (finalized: true) وتنتمي لهذا الشهر
